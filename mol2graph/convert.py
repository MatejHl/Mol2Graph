...
def mol_to_spektral(mol, scipy_E = False):
    """
    For now, this is just an example function. Copy it and change if you need 
    any other Atom/Bond attributes.
    ...
    if scipy_E:
        ...
        E = scipy.sparse.coo_matrix((data + data, (begin + end, end + begin)), shape = Chem.GetAdjacencyMatrix(mol).shape)
    else:
        E = np.array(E)

    G = sp_Graph(x = np.array(X),
                a = A,
                e = E,
                y = None)
    return G
...
def smiles_to_spektral(smiles, validate=False, scipy_E = False):
    """
    Convert SMILES string to nx.Graph.
    ...
    mol = Chem.MolFromSmiles(smiles.strip())
    can_smi = Chem.MolToSmiles(mol) # canonical SMILES - TO DO: Check if this row is necessary.
    G = mol_to_spektral(mol, scipy_E = scipy_E)
    if validate:
